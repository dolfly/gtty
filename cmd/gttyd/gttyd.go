...
func main() {
	...
	app.Action = func(c *cli.Context) {
		...
		if configFile != "~/.gttyd" || !os.IsNotExist(err) {
		...
		srv, err := gtty.New(factory, appOptions)
		if err != nil {
			exit(err, 3)
		}
		...
		log.Printf("GTTYD is starting with command: %s", strings.Join(args, " "))

		errs := make(chan error, 1)
		go func() {
			errs <- srv.Run(ctx, gtty.WithGracefullContext(gCtx))
		}()
